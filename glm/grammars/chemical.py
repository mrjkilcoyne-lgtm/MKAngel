...
def build_bonding_grammar() -> Grammar:
    ...
    rules = [
        ...
        # --- VSEPR shapes ---
        Rule(name="vsepr_trigonal_planar",
             pattern={"steric_number": 3, "lone_pairs": 0},
             result={"geometry": "trigonal planar", "bond_angle": "120°",
                     "examples": ["BF₃", "CO₃²⁻", "NO₃⁻"]},
             weight=1.0, direction="bidirectional"),
        Rule(name="vsepr_tetrahedral",
             pattern={"steric_number": 4, "lone_pairs": 0},
             result={"geometry": "tetrahedral", "bond_angle": "109.5°",
                     "examples": ["CH₄", "NH₄⁺", "SO₄²⁻"]},
             weight=1.0, direction="bidirectional"),
        Rule(name="vsepr_trigonal_bipyramidal",
             pattern={"steric_number": 5, "lone_pairs": 0},
             result={"geometry": "trigonal bipyramidal", "bond_angles": "90° and 120°",
                     "examples": ["PCl₅", "PF₅"]},
             weight=1.0, direction="bidirectional"),
        Rule(name="vsepr_octahedral",
             pattern={"steric_number": 6, "lone_pairs": 0},
             result={"geometry": "octahedral", "bond_angle": "90°",
                     "examples": ["SF₆", "Mo(CO)₆"]},
             weight=1.0, direction="bidirectional"),
        Rule(name="vsepr_bent",
             pattern={"steric_number": 4, "lone_pairs": 2},
             result={"geometry": "bent/angular", "bond_angle": "≈104.5°",
                     "example": "H₂O"},
             weight=1.0, direction="bidirectional"),
        # --- Bond types ---
        Rule(name="hydrogen_bonding",
             pattern={"condition": "H bonded to F, O, or N; adjacent lone pair"},
             result={"interaction": "hydrogen bond (10-40 kJ/mol)",
                     "consequence": "water's high boiling point, DNA base pairing, protein folding",
                     "isomorphism": "like prosodic binding across word boundaries in phonology"},
             weight=1.0, direction="forward"),
        Rule(name="metallic_bonding",
             pattern={"material": "metal atoms with delocalised electrons"},
             result={"model": "electron sea — cations in delocalised electron cloud",
                     "properties": "electrical conductivity, malleability, lustre"},
             weight=1.0, direction="forward"),
        # --- Periodic table ---
        Rule(name="aufbau_principle",
             pattern={"electrons": "filling atomic orbitals"},
             result={"order": "1s 2s 2p 3s 3p 4s 3d 4p 5s 4d 5p 6s 4f 5d 6p 7s 5f 6d 7p",
                     "basis": "increasing energy, Madelung rule (n+l then n)",
                     "consequence": "structure of periodic table"},
             weight=1.0, direction="forward"),
        Rule(name="hunds_rule",
             pattern={"orbitals": "degenerate orbitals being filled"},
             result={"rule": "maximise total spin — fill each orbital singly before pairing",
                     "basis": "exchange energy minimisation"},
             weight=1.0, direction="forward"),
    ]
...
def build_reaction_grammar() -> Grammar:
    ...
    rules = [
        ...
        # --- Arrhenius equation ---
        Rule(name="arrhenius_equation",
             pattern={"process": "temperature-dependent reaction rate"},
             result={"equation": "k = A·exp(-Eₐ/RT)",
                     "A": "pre-exponential factor (collision frequency × orientation)",
                     "Ea": "activation energy (kJ/mol)",
                     "verified": "Arrhenius 1889; universal for elementary reactions"},
             weight=1.0, direction="forward"),
        # --- Rate laws ---
        Rule(name="zero_order_rate",
             pattern={"kinetics": "rate independent of concentration"},
             result={"rate_law": "rate = k", "integrated": "[A] = [A]₀ - kt",
                     "half_life": "t½ = [A]₀/(2k)"},
             weight=1.0, direction="forward"),
        Rule(name="first_order_rate",
             pattern={"kinetics": "rate proportional to [A]"},
             result={"rate_law": "rate = k[A]", "integrated": "ln[A] = ln[A]₀ - kt",
                     "half_life": "t½ = ln2/k (constant, independent of [A]₀)",
                     "examples": "radioactive decay, unimolecular decomposition"},
             weight=1.0, direction="bidirectional"),
        Rule(name="second_order_rate",
             pattern={"kinetics": "rate proportional to [A]²"},
             result={"rate_law": "rate = k[A]²", "integrated": "1/[A] = 1/[A]₀ + kt",
                     "half_life": "t½ = 1/(k[A]₀)"},
             weight=1.0, direction="forward"),
        # --- Le Chatelier ---
        Rule(name="le_chatelier",
             pattern={"system": "equilibrium disturbed by change"},
             result={"principle": "system shifts to partially counteract the disturbance",
                     "examples": ["increase [reactant] → shift to products",
                                  "increase temperature → shift toward endothermic direction",
                                  "increase pressure → shift toward fewer moles of gas"],
                     "verified": "Le Chatelier 1884; universal for equilibria"},
             weight=1.0, direction="forward"),
        # --- Hess's law ---
        Rule(name="hess_law",
             pattern={"thermochemistry": "multi-step reaction"},
             result={"law": "ΔH_total = Σ ΔH_steps (enthalpy is a state function)",
                     "consequence": "can calculate ΔH for reactions that can't be measured directly",
                     "verified": "Hess 1840"},
             weight=1.0, direction="bidirectional"),
        # --- Nernst equation ---
        Rule(name="nernst_equation",
             pattern={"electrochemistry": "cell potential at non-standard conditions"},
             result={"equation": "E = E° - (RT/nF)ln(Q)",
                     "at_25C": "E = E° - (0.0592/n)log(Q)",
                     "equilibrium": "when E = 0, Q = K"},
             weight=1.0, direction="bidirectional"),
        # --- Acid-base ---
        Rule(name="ph_definition",
             pattern={"aqueous_solution": "hydrogen ion concentration [H⁺]"},
             result={"equation": "pH = -log₁₀[H⁺]",
                     "water_autoionisation": "Kw = [H⁺][OH⁻] = 10⁻¹⁴ at 25°C",
                     "neutral": "pH = 7 at 25°C"},
             weight=1.0, direction="bidirectional"),
        Rule(name="henderson_hasselbalch",
             pattern={"buffer": "weak acid HA and conjugate base A⁻"},
             result={"equation": "pH = pKa + log([A⁻]/[HA])",
                     "use": "calculate pH of buffer solutions"},
             weight=1.0, direction="bidirectional"),
    ]
...
def build_molecular_grammar() -> Grammar:
    ...
    rules = [
        ...
        # --- Gas laws ---
        Rule(name="ideal_gas_law_chem",
             pattern={"gas": "ideal (low P, high T)"},
             result={"equation": "PV = nRT",
                     "R": "8.314 J/(mol·K) = 0.08206 L·atm/(mol·K)"},
             weight=1.0, direction="bidirectional"),
        Rule(name="daltons_law",
             pattern={"mixture": "non-reacting ideal gases"},
             result={"law": "P_total = Σ P_i (sum of partial pressures)",
                     "partial_pressure": "P_i = x_i × P_total (mole fraction)"},
             weight=1.0, direction="bidirectional"),
        Rule(name="grahams_law",
             pattern={"process": "gaseous effusion or diffusion"},
             result={"equation": "rate₁/rate₂ = √(M₂/M₁)",
                     "consequence": "lighter molecules effuse faster"},
             weight=1.0, direction="forward"),
        # --- Spectroscopy ---
        Rule(name="beer_lambert_law",
             pattern={"spectroscopy": "light absorption through solution"},
             result={"equation": "A = εlc",
                     "A": "absorbance", "epsilon": "molar absorptivity (L/(mol·cm))",
                     "l": "path length (cm)", "c": "concentration (mol/L)",
                     "verified": "Beer 1852, Lambert 1760"},
             weight=1.0, direction="bidirectional"),
        # --- Colligative properties ---
        Rule(name="boiling_point_elevation",
             pattern={"solution": "non-volatile solute in solvent"},
             result={"equation": "ΔTb = i·Kb·m",
                     "Kb_water": "0.512 °C·kg/mol",
                     "basis": "solute lowers vapour pressure → higher boiling point"},
             weight=1.0, direction="forward"),
        Rule(name="freezing_point_depression",
             pattern={"solution": "solute in solvent"},
             result={"equation": "ΔTf = i·Kf·m",
                     "Kf_water": "1.86 °C·kg/mol",
                     "application": "road salt, antifreeze"},
             weight=1.0, direction="forward"),
        # --- Raoult's law ---
        Rule(name="raoults_law",
             pattern={"solution": "ideal liquid mixture"},
             result={"equation": "P_A = x_A × P°_A",
                     "meaning": "vapour pressure proportional to mole fraction",
                     "deviation": "non-ideal solutions deviate (positive or negative)"},
             weight=1.0, direction="bidirectional"),
    ]
